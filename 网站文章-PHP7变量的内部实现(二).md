...
使用opcache时，内存使用量为32MiB，但没有opcache时使用量会增加到390MB，因为在这种情况下，$array的每个元素都将获得['foo']的新副本。在这里完成实际复制（而不是引用refcount）的原因是文字虚拟操作数不使用引用计数来避免SHM损坏。我希望我们能够改善这个目前的灾难性案例，以便将来在没有opcache的情况下也能更好地运行。
